    ...
    setup() {
        ...
        const fetchData = async () => {
            try {
                ...
                const totalCountResponse = await axios.get('http://localhost:3000/students/total-count');
                totalStudents.value = totalCountResponse.data.totalCount;
                handleSearch();
            } catch (error) {
        ...
        const handleRowClick = async(row) => {
            try{
                const response = await axios.get(`http://localhost:3000/students/students_by_stuid/${row.stu_id}`);
